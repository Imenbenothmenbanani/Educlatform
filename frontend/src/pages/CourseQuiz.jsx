...
const generateQuiz = async () => {
  try {
    ...
    // ⚡️ Utilisation du mock à la place de l'API
    const mockQuiz = {
      questions: Array.from({ length: numberOfQuestions }, (_, i) => ({
        id: i + 1,
        question: `Question mock ${i + 1}`,
        options: ['A', 'B', 'C', 'D'],
        correctAnswer: Math.floor(Math.random() * 4),
        explanation: 'Ceci est une explication mock.'
      }))
    };
    setQuiz(mockQuiz);

    // Si tu veux tester l'API plus tard, tu peux garder ce code commenté
    /*
